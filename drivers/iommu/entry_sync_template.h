				...
				quanta_t *unused_update, quanta_t *memory)
{
	...
	for (i = 0; i != writer->num_quantas; i++) {
		/*
		 * Check that masks are up to date, the driver configuration
		 * functions are not allowed to set a bit to 1 if the used
		 * function doesn't say it is used.
		 */
		...
		if ((unused_update[i] & target_used[i]) != target[i])
			used_quanta_diff |= 1 << i;
	}
	return used_quanta_diff;
}
			  ...
			  size_t memory_len)
{
	...
	used_quanta_diff = NS(entry_quanta_diff)(writer, entry, target,
						unused_update, memory);
	if (hweight8(used_quanta_diff) == 1) {
		/*
		 ...
		 */
		unsigned int critical_qword_index = ffs(used_quanta_diff) - 1;
