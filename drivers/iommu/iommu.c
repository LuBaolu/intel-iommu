...
/*
 * Returns 0 on success and an appropriate error code otherwise. Specifically,
 * -ENOSYS is returned if a fault handler isn't installed (though fault
 * handlers can also return -ENOSYS, in case they want to elicit the default
 * behavior of the IOMMU drivers).
 */
					    ...
					    struct mm_struct *mm)
{
	...
	mmgrab(mm);
	domain->mm = mm;
	iommu_set_iopage_fault_handler(domain, iommu_sva_handle_iopf, mm);
